...
pub enum Message {
    ...
    /// Sender offers to transfer notes (no data yet — just metadata).
    TransferOffer {
        from_peer: String,
        offer_id: String,
        note_count: u32,
    },
    /// Recipient accepts the offer by sending back the pairing code.
    TransferAccept {
        offer_id: String,
        passphrase: String,
    },
...
mod tests {
    ...
    fn transfer_accept_roundtrip() {
        let msg = Message::TransferAccept {
            offer_id: "offer-1".into(),
            passphrase: "K4X7P2".into(),
        };
        let decoded = roundtrip(&msg);
        assert!(matches!(decoded, Message::TransferAccept { passphrase, .. } if passphrase == "K4X7P2"));
    }
